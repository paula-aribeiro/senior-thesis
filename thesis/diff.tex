...
%DIF DEL thesis.tex                         Wed Nov 28 13:17:56 2018
%DIF ADD thesis_rev_Arthur_2018-11-29.tex   Fri Nov 30 18:15:25 2018
...
%DIF 15a15
\usepackage{amssymb}  %Pacote com complementos do modo matemático. %DIF > 
%DIF -------
...
O ferro \DIFaddbegin \DIFadd{puro }\DIFaddend em estado sólido tem duas formas alotrópicas, ou seja, diferentes estruturas cristalinas que dependem da temperatura e pressão. A baixas temperaturas, o ferro assume a estrutura cúbica de corpo centrado (CCC) e é denominado $\alpha$-Fe, ou ferrita. Acima de 910\DIFaddbegin \DIFadd{~}\DIFaddend °C, \DIFdelbegin \DIFdel{os cristais transformam sua estrutura }\DIFdelend \DIFaddbegin \DIFadd{a disposição atômica do ferro muda de CCC }\DIFaddend para cúbica de faces centradas (CFC), também chamada de $\gamma$-Fe, ou austenita\DIFdelbegin \DIFdel{, de caráter paramagnético}\DIFdelend . A estabilidade da austenita permanece até 1400\DIFaddbegin \DIFadd{~}\DIFaddend °C, quando volta a assumir uma estrutura CCC\DIFdelbegin \DIFdel{de caráter paramagnético}\DIFdelend . Esta ferrita de alta temperatura é \DIFdelbegin \DIFdel{comummente }\DIFdelend \DIFaddbegin \DIFadd{comumente }\DIFaddend chamada de $\delta$-Fe \DIFdelbegin \DIFdel{para diferenciar a }\DIFdelend \DIFaddbegin \DIFadd{devido à diferente }\DIFaddend faixa de temperatura de ocorrência \DIFaddbegin \DIFadd{da fase $\alpha-Fe$}\DIFaddend . Alguns autores também diferenciam a fase $\beta$-Fe da fase $\alpha$-Fe, ambas de estrutura CCC, pelo fato de que para temperaturas superiores a 770\DIFaddbegin \DIFadd{~}\DIFaddend °C (temperatura de Curie) o $\alpha$-Fe perde suas propriedades ferromagnéticas e passa a ser paramagnético \cite{Totten2006}.

A partir da combinação dessas possíveis estruturas do ferro com outros elementos formam-se as ligas. Como o ferro é a base do aço e tem estruturas cristalinas limitadas, é a sua combinação com outros átomos que resulta em diferentes propriedades. O carbono possui baixa solubilidade na fase $\alpha$ (0,02\% em massa a 738\DIFaddbegin \DIFadd{~}\DIFaddend °C), mas é bastante solúvel na fase $\gamma$, pois a estrutura CFC permite a alocação \DIFdelbegin \DIFdel{dos }\DIFdelend \DIFaddbegin \DIFadd{de uma maior fração de }\DIFaddend átomos de carbono em seus interstícios.
...
Um ponto importante do diagrama \DIFaddbegin \DIFadd{mostrado na Figura \ref{fig:diagrama_fe-c_meta} }\DIFaddend é o ponto eutetóide, no qual \DIFdelbegin \DIFdel{as três fases coexistem}\DIFdelend \DIFaddbegin \DIFadd{coexistem em equilíbrio as fases $\alpha$, $\gamma$ e \ch{Fe3C}}\DIFaddend . Para um sistema apenas ferro e carbono, \DIFdelbegin \DIFdel{ele corresponde a }\DIFdelend \DIFaddbegin \DIFadd{o teor de carbono correspondente a esta fase é }\DIFaddend aproximadamente 0,8\% C em massa. \DIFdelbegin \DIFdel{Assim, aços de }\DIFdelend \DIFaddbegin \DIFadd{Ligas com teores de carbono inferiores a esta }\DIFaddend composição \DIFdelbegin \DIFdel{abaixo desse ponto }\DIFdelend são \DIFdelbegin \DIFdel{denominados }\DIFdelend \DIFaddbegin \DIFadd{denominadas }\DIFaddend hipoeutetóides e, \DIFdelbegin \DIFdel{acima, }\DIFdelend \DIFaddbegin \DIFadd{ligas com teores de carbono superiores à do ponto eutetóide são chamadas de }\DIFaddend hipereutetóides.

\section{Decomposição da Austenita}

\DIFdelbegin \DIFdel{Para um aço resfriado lentamente à }\DIFdelend \DIFaddbegin \textbf{\DIFadd{PENSANDO BEM, ACHO QUE VOCÊ NÃO PRECISA DESSA SEÇÃO. SEU TRABALHO É SOBRE TRANSFORMAÇÕES NO EQUILÍBRIO METAESTÁVEL}}

\DIFadd{Para uma liga Fe-C resfriada lentamente até a }\DIFaddend temperatura ambiente, o teor de carbono \DIFdelbegin \DIFdel{controla as }\DIFdelend \DIFaddbegin \DIFadd{é fator determinante na natureza e proporção da }\DIFaddend fases formadas. \DIFdelbegin \DIFdel{Entretanto}\DIFdelend \DIFaddbegin \DIFadd{Por sua vez}\DIFaddend , a taxa de resfriamento controla a formação de constituintes metaestáveis. Essa formação de constituintes a partir da austenita pode ocorrer por difusão, cisalhamento ou uma mistura dos dois mecanismos \cite{Honeycombe1982}.
...
\DIFdelbegin \DIFdel{O francês }\DIFdelend Le Chatelier foi o primeiro a atribuir a letra ``A'' para as temperaturas críticas de transformação, devido à palavra \textit{Arrêt}, que representa a parada na temperatura durante a transformação de fase \cite{Silva2010}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Para exemplificar essas temperaturas críticas em aços, \DIFdelbegin \DIFdel{utilizou-se dados de chamadas do }\DIFdelend \DIFaddbegin \DIFadd{são mostrados a seguir gráficos obtidos por simulações de termodinâmica computacional determinados utilizando o }\DIFaddend software Thermo-Calc\textregistered{} \DIFdelbegin \DIFdel{para traçar o diagrama Fe-C metaestável para um aço 1}\DIFdelend \DIFaddbegin \DIFadd{correspondentes a uma liga Fe-1}\DIFaddend \%\DIFdelbegin \DIFdel{Mn, em massa , da Figura \ref{fig:fe-1mn-C_isopleth}, bem como os gráficos de fração molar de fase em função da temperatura para aços hipo, hiper e eutetóide, das Figuras \ref{fig:fe-01c-1mn}, \ref{fig:fe-0727c-1mn} e \ref{fig:fe-1c-1mn} a seguir. }%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[ht!]
%DIFDELCMD <   \includegraphics[width=.9\textwidth]{img/Fe-1Mn-C_isopleth_edited.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Diagrama de fases Fe-C para aço 1\% Mn, em massa}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:fe-1mn-C_isopleth}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Mn-C.
}\DIFaddend 

\DIFaddbegin \DIFadd{A Figura \ref{fig:fe-1mn-C_isopleth} corresponde à isopleta do carbono para o teor de 1\% em massa de Mn. As linhas tracejas na vertical representam três ligas com teores de carbono também fixos. }\DIFaddend A primeira linha vertical tracejada da Figura \ref{fig:fe-1mn-C_isopleth} representa um aço hipoeutetóide e \DIFdelbegin \DIFdel{sua }\DIFdelend \DIFaddbegin \DIFadd{o gráfico da }\DIFaddend fração molar \DIFdelbegin \DIFdel{de fases pode ser vista na Figura \ref{fig:fe-01c-1mn}. }%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[ht!]
%DIFDELCMD <   \includegraphics[width=1.1\textwidth]{img/Fe-Mn-01C_edited.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Fração molar de fase versus Temperatura para aço 0,1\%C 1\% Mn, em massa}}
  %DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:fe-01c-1mn}%%%
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{das fases em função da temperatura é apresentado na Figura \ref{fig:fe-1mn-c}a. }\DIFaddend Um aço \DIFdelbegin \DIFdel{nessa }\DIFdelend \DIFaddbegin \DIFadd{com essa }\DIFaddend composição, mediante aquecimento, mantém as fases $\alpha$ e \ch{Fe3C} \DIFaddbegin \DIFadd{estáveis }\DIFaddend até \DIFdelbegin \DIFdel{chegar ao ponto }\DIFdelend \DIFaddbegin \DIFadd{a temperatura }\DIFaddend A1 \DIFdelbegin \DIFdel{, no }\DIFdelend \DIFaddbegin \DIFadd{ser atingida, na }\DIFaddend qual a fase $\alpha$ começa a se decompor em $\gamma$. No gráfico da Figura \DIFdelbegin \DIFdel{\ref{fig:fe-01c-1mn}, esse ponto pode ser notado }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:fe-1mn-c}a, essa temperatura pode ser notada }\DIFaddend pela primeira mudança de inclinação da curva da austenita. O aço permanece no campo trifásico \DIFaddbegin \DIFadd{($\alpha$ + $\gamma$ + \ch{Fe3C}) }\DIFaddend até \DIFdelbegin \DIFdel{encontrar o ponto }\DIFdelend \DIFaddbegin \DIFadd{a temperatura }\DIFaddend A1' \DIFaddbegin \DIFadd{ser atingida}\DIFaddend , quando toda a fase \ch{Fe3C} \DIFdelbegin \DIFdel{se transforma em $\gamma$, a segunda mudança de inclinação na curva da austenita. Caso o aquecimento permaneça}\DIFdelend \DIFaddbegin \DIFadd{é consumida. Para temperaturas crescentes}\DIFaddend , a fase $\alpha$ \DIFdelbegin \DIFdel{começa a se transformar }\DIFdelend \DIFaddbegin \DIFadd{se transforma }\DIFaddend em $\gamma$ \DIFdelbegin \DIFdel{, }\DIFdelend até \DIFdelbegin \DIFdel{atingir o ponto }\DIFdelend \DIFaddbegin \DIFadd{a temperatura }\DIFaddend A3 \DIFdelbegin \DIFdel{, onde }\DIFdelend \DIFaddbegin \DIFadd{ser atingida, na qual }\DIFaddend tem-se 100\% de fase $\gamma$\DIFdelbegin \DIFdel{, a última mudança de inclinação na curva da austenita}\DIFdelend .

\DIFaddbegin \begin{figure}[ht!]
  \includegraphics[width=.9\textwidth]{img/Fe-1Mn-C_isopleth_edited.png}
  \caption{\DIFaddFL{Diagrama de fases Fe-C para aço 1\% Mn, em massa}}
...
\begin{figure}[ht!]
  ...
  \caption{\DIFaddFL{Fração molar de fase versus Temperatura para aço 1\%C 1\% Mn, em massa}}
  \label{fig:fe-1mn-c}
\end{figure}

\DIFaddend Um aço eutetóide, correspondente à segunda linha vertical \DIFdelbegin \DIFdel{tracejada }\DIFdelend da Figura \ref{fig:fe-1mn-C_isopleth} e ao gráfico da Figura \DIFdelbegin \DIFdel{\ref{fig:fe-0727c-1mn}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:fe-1mn-c}b}\DIFaddend , segue o mesmo raciocínio. A diferença é que não há um campo trifásico \DIFdelbegin \DIFdel{, por isso a }\DIFdelend \DIFaddbegin \DIFadd{e, portanto, não há mais uma }\DIFaddend temperatura A1' \DIFdelbegin \DIFdel{equivale à temperatura A3}\DIFdelend \DIFaddbegin \DIFadd{definida}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{figure}[ht!]
%DIFDELCMD <   \includegraphics[width=1.1\textwidth]{img/Fe-Mn-0727C_edited.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Fração molar de fase versus Temperatura para aço 0,727\%C 1\% Mn, em massa}}
  %DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:fe-0727c-1mn}%%%
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Já para um aço hipereutetóide, terceira linha vertical \DIFdelbegin \DIFdel{tracejada da }\DIFdelend \DIFaddbegin \DIFadd{na }\DIFaddend Figura \ref{fig:fe-1mn-C_isopleth} e gráfico da Figura \DIFdelbegin \DIFdel{\ref{fig:fe-1c-1mn}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:fe-1mn-c}c}\DIFaddend , a diferença é que o segundo campo bifásico é constituído por $\gamma$ e \ch{Fe3C}, até atingir o ponto em que toda a \DIFaddbegin \DIFadd{fase }\DIFaddend \ch{Fe3C} se transforma em $\gamma$. \DIFdelbegin \DIFdel{Certas literaturas}\DIFdelend \DIFaddbegin \DIFadd{Algumas referências}\DIFaddend , como \citaremsentenca{Digges1960}, fazem distinção para a temperatura A3 de aços hipereutetóides, chamando-a de Acm, devido à diferença de campos bifásicos. No presente trabalho, ambas serão chamadas de A3, por corresponderem à \DIFdelbegin \DIFdel{mínima }\DIFdelend \DIFaddbegin \DIFadd{menor }\DIFaddend temperatura em que a fração de austenita é \DIFdelbegin \DIFdel{igual a 1.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[ht!]
%DIFDELCMD <   \includegraphics[width=1.1\textwidth]{img/Fe-Mn-1C_edited.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Fração molar de fase versus Temperatura para aço 1\%C 1\% Mn, em massa}}
  %DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:fe-1c-1mn}%%%
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{100\%.
}\DIFaddend 

Em resumo, pode-se dizer que a temperatura A1 corresponde à máxima temperatura em que a fração de austenita é zero, enquanto a A3 é a mínima temperatura cuja fração de austenita é \DIFdelbegin \DIFdel{um }\DIFdelend \DIFaddbegin \DIFadd{100\% }\DIFaddend e A1' é o limite superior do campo intercrítico de três fases \cite{Honeycombe1982}.

É possível ainda diferenciar a temperatura crítica no resfriamento da de aquecimento, utilizando respectivamente as letras ``r'' e ``e''. \DIFdelbegin \DIFdel{Em teoria, elas deveriam ser iguais, mas na }\DIFdelend \DIFaddbegin \DIFadd{Sob aquecimento e resfriamento lentos (ou seja, sob condições de equilíbrio) elas devem ser iguais. Na }\DIFaddend prática\DIFdelbegin \DIFdel{a taxa }\DIFdelend \DIFaddbegin \DIFadd{, as taxas }\DIFaddend de resfriamento ou aquecimento \DIFdelbegin \DIFdel{diferencia }\DIFdelend \DIFaddbegin \DIFadd{aplicadas deslocam as temperaturas }\DIFaddend Ae1 de Ar1 e Ae3 de Ar3 \DIFaddbegin \DIFadd{do equilíbrio, devido às cinéticas de formação e dissolução das fases}\DIFaddend . A faixa de temperatura entre A1 e A3 é chamada de intervalo crítico ou de transformação \cite{Digges1960}.

\section{Tratamento térmico de aços}

A \DIFaddbegin \DIFadd{importância da determinação das temperaturas críticas está diretamente relacionada à aplicação de tratamentos térmicos a aços. A }\DIFaddend obtenção das propriedades ideais de um aço está relacionada tanto com sua composição química quanto com os processos de tratamento térmico aos quais ele é submetido \cite{Totten2006}. Tratamentos térmicos podem ser utilizados para aumentar ou diminuir a ductilidade, dureza, tensão de escoamento ou tenacidade, otimizando essas propriedades para a finalidade do material \cite{Silva2010}.

A austenitização é a etapa que precede um tratamento térmico e consiste em aquecer o aço a uma temperatura em que haja formação da austenita. Esta pode ser parcial, quando se encontra na faixa de transformação \DIFaddbegin \DIFadd{(ou seja, entre as temperaturas A1 e A3)}\DIFaddend , ou total, quando está acima do intervalo de transformação \DIFaddbegin \DIFadd{(acima da temperatura A3) }\DIFaddend \cite{ASM1991}.
...
\DIFaddend  \begin{itemize} 
  \item Classe 1: elementos de \DIFdelbegin \DIFdel{domínio-}\DIFdelend \DIFaddbegin \DIFadd{domínio }\DIFaddend $\gamma$ aberto \DIFaddbegin \DIFadd{(Figura \ref{fig:elem-liga}a)}\DIFaddend , podendo até mesmo eliminar completamente a fase $\alpha$ em concentrações suficientemente altas. Assim, as transformações $\gamma \rightarrow  \alpha$ ocorrem a temperaturas menores, ou seja, A3 diminui e pode haver casos em que não há a temperatura A1. Fazem parte desse grupo: Níquel, Manganês, Cobalto, Rutênio, Ródio, Paládio, Ósmio, Irídio e Platina.

  \item Classe 2: \DIFdelbegin \DIFdel{domínio-}\DIFdelend \DIFaddbegin \DIFadd{elementos de domínio }\DIFaddend $\gamma$ \DIFdelbegin \DIFdel{é expandido }\DIFdelend \DIFaddbegin \DIFadd{expandido (Figura \ref{fig:elem-liga}b) }\DIFaddend até a formação de um composto de ferro. Essa expansão é responsável por formar solução sólida homogênea, sendo muito importante para o tratamento térmico dos aços. Pertencem a esse grupo o Carbono e o Nitrogênio, que diminuem o valor de A3.

  \item Classe 3: \DIFdelbegin \DIFdel{domínio-}\DIFdelend \DIFaddbegin \DIFadd{domínio }\DIFaddend $\gamma$ fechado \DIFaddbegin \DIFadd{(Figura \ref{fig:elem-liga}c)}\DIFaddend , elementos que favorecem a expansão do \DIFdelbegin \DIFdel{domínio-}\DIFdelend \DIFaddbegin \DIFadd{domínio }\DIFaddend $\alpha$, que circunda o campo austenítico, formando uma região chamada de ilha gama ou $\gamma$-loop. Estes aumentam A1 e pode haver casos em que A3 não existe. Essas ligas não podem passar por tratamento térmico de arrefecimento através da transformação $\gamma \rightarrow \alpha$. Fazem parte desse grupo: Silício, Alumínio, Berílio, Fósforo e elementos fortemente formadores de carboneto, como Titânio, Vanádio, Molibdênio e Cromo.

  \item Classe 4: \DIFdelbegin \DIFdel{domínio-}\DIFdelend \DIFaddbegin \DIFadd{domínio }\DIFaddend $\gamma$ contraído mas há formação de compostos de ferro \DIFaddbegin \DIFadd{(Figura \ref{fig:elem-liga}d)}\DIFaddend . Os elementos Boro, Enxofre, Tântalo, Nióbio, Zircônio estão nessa categoria \cite{Honeycombe1982} \cite{Silva2010}.
 \end{itemize} 

\DIFdelbegin \DIFdel{A Figura \ref{fig:elem-liga} mostra a influência dos elementos de liga no campo austenítico em diagramas binários de ferro.
}\DIFdelend %DIF >  A Figura \ref{fig:elem-liga} mostra a influência dos elementos de liga no campo austenítico em diagramas binários de ferro.

\begin{figure}[ht!]
  \includegraphics[width=.7\textwidth]{img/elementos-liga.png}
  \caption{Classificação dos \DIFdelbeginFL \DIFdelFL{domínios-}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{domínios }\DIFaddendFL $\gamma$ sob influência dos elementos de liga: a) aberto; b) expandido; c) fechado; d) contraído. \cite{Honeycombe1982}}
  \label{fig:elem-liga}
\end{figure}

\section{Determinação das Temperaturas Críticas}

\DIFdelbegin \DIFdel{Dada a importância de conhecer as }\DIFdelend %DIF >  Dada a importância de conhecer as temperaturas críticas de aços para obtenção das propriedades desejadas, foram desenvolvidas diversas formas para determiná-las.
\DIFaddbegin \DIFadd{São reportadas na literatura diversas formas de determinação de }\DIFaddend temperaturas críticas de aços\DIFdelbegin \DIFdel{para obtenção das propriedades desejadas, foram desenvolvidas diversas formas para determiná-las}\DIFdelend \DIFaddbegin \DIFadd{, sejam de forma experimental ou computacional}\DIFaddend .

\DIFdelbegin \DIFdel{Como as transformações }\DIFdelend \DIFaddbegin \DIFadd{Transformações }\DIFaddend de fase do aço \DIFdelbegin \DIFdel{também }\DIFdelend causam contração ou expansão \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{do material, devido às diferentes densidades das fases que se formam ou dissolvem. A dilatometria é }\DIFaddend um método de determinação experimental \DIFdelbegin \DIFdel{é }\DIFdelend \DIFaddbegin \DIFadd{de detecção de transformações de fases }\DIFaddend através da \DIFdelbegin \DIFdel{dilatometria sob aquecimento contínuo. O dilatômetro coleta }\DIFdelend \DIFaddbegin \DIFadd{coleta de }\DIFaddend sinais de mudança nas dimensões do corpo de prova bem como sua temperatura\DIFdelbegin \DIFdel{, para traçar a curva de derivada da dimensão relativa }\DIFdelend \DIFaddbegin \DIFadd{. As temperaturas críticas de transformação podem então ser determinadas graficamente pelas inflexões nas curvas da dilatação }\DIFaddend em função da temperatura, como as \DIFdelbegin \DIFdel{do exemplo }\DIFdelend \DIFaddbegin \DIFadd{mostradas }\DIFaddend na Figura \ref{fig:dilatometer}. Na \DIFdelbegin \DIFdel{literatura utilizada por }%DIFDELCMD < \citaremsentenca{Pawlowski2012}%%%
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{Figura \ref{fig:dilatometer}, na nomenclatura utilizada pelo autor, a temperatura }\DIFaddend A1 equivale a A\textsubscript{c1p}, A1' \DIFaddbegin \DIFadd{corresponde }\DIFaddend a A\textsubscript{c1k}, e \DIFaddbegin \DIFadd{a temperatura }\DIFaddend A3 \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{é a temperatura }\DIFaddend Ac\textsubscript{3} ou Acm. Apesar de \DIFdelbegin \DIFdel{muito }\DIFdelend preciso, o equipamento tem custo elevado e necessita de pessoas capacitadas para operá-lo.
...
Outro método de determinar as temperaturas críticas de transformação é através \DIFdelbegin \DIFdel{do }\DIFdelend \DIFaddbegin \DIFadd{de softwares de termodinâmica computacional, como o }\DIFaddend Thermo-Calc\textregistered{}, \DIFdelbegin \DIFdel{software utilizado para diversos cálculos }\DIFdelend \DIFaddbegin \DIFadd{que calculam as variáveis de estado (e.g., fração de fases, composição das fases) baseados nos princípios }\DIFaddend termodinâmicos \DIFdelbegin \DIFdel{baseado na }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend minimização da energia \DIFdelbegin \DIFdel{de Gibbs. Ele tem acesso a diversos bancos }\DIFdelend \DIFaddbegin \DIFadd{livre de Gibbs). Estes softwares acessam bases }\DIFaddend de dados termodinâmicos \DIFdelbegin \DIFdel{, em especial os desenvolvidos pelo }\textit{\DIFdel{Scientific Group Thermodata Europe}} %DIFAUXCMD
\DIFdel{(SGTE). Sendo um resultado de mais de 30 anos de coleta de dados, sua }\DIFdelend \DIFaddbegin \DIFadd{que fornecem informações dos parâmetros de interação dos elementos químicos para determinadas fases, que então são utilizados para o cálculo da energia livre de Gibbs. A }\DIFaddend precisão \DIFaddbegin \DIFadd{dos cálculos computacionais }\DIFaddend é \DIFdelbegin \DIFdel{inquestionável \mbox{%DIFAUXCMD
\cite{TC2002}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{precisa e tem sido constantemente avaliada na literatura}\DIFaddend . Entretanto, \DIFdelbegin \DIFdel{trata-se de um software pago e requer }\DIFdelend \DIFaddbegin \DIFadd{tanto software quanto o acesso aos bancos de dados é normalmente pago, e requer-se }\DIFaddend certo aprendizado para sua manipulação.

Uma terceira maneira é a utilização de equações empíricas que se baseiam na concentração em massa dos elementos presentes no aço. A elaboração dessas equações envolve um método \DIFdelbegin \DIFdel{numérico }\DIFdelend de regressão múltipla. \DIFdelbegin %DIFDELCMD < \citaremsentenca{ANDREWS1965} \enfase{apud} %%%
\DIFdelend \citaremsentenca{Gorni2012} \DIFdelbegin \DIFdel{publicou }\DIFdelend \DIFaddbegin \DIFadd{compilou diversas }\DIFaddend fórmulas para o cálculo das temperaturas de transformação para austenita A1 e A3\DIFdelbegin \DIFdel{, descritas nas }\DIFdelend \DIFaddbegin \DIFadd{. Abaixos são mostradas duas }\DIFaddend equações \DIFdelbegin \DIFdel{\ref{eq:andrewsA1} e \ref{eq:andrewsA3} a seguir.
}\DIFdelend \DIFaddbegin \DIFadd{para o cálculo das temperaturas A1 e A3, conforme propostas por }\citaremsentenca{ANDREWS1965}\DIFadd{:
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Ambos }\DIFdelend %DIF >  REGRESSÃO E INTERPOLAÇÃO SÃO COISAS DISTINTAS
%DIF >  O primeiro e mais simples é a interpolação polinomial. Este pode se comportar de forma linear, como descrito na equação \ref{eq:interpolacao_linear}, ou quadrática, como na equação \ref{eq:interpolacao_quadratica}.
%DIF >  Este pode se comportar de forma linear, como descrito na equação \ref{eq:interpolacao_linear}, ou quadrática, como na equação \ref{eq:interpolacao_quadratica}.
\DIFaddbegin 

%DIF >  \begin{align}
%DIF >    f(x) &= b x + c \label{eq:interpolacao_linear} \\
%DIF >    f(x) &= \frac{1}{2} a x^T + b x + c \label{eq:interpolacao_quadratica}
%DIF >  \end{align}

%DIF >  Ambos são métodos muito utilizados devido ao baixo custo computacional. Entretanto, têm a necessidade de estabelecer alguma relação (linear ou quadrática) entre os termos estudados, utilizando termos fixos \cite{Bhadeshia1999}.

%DIF >  USE O CAPÍTULO 10 DO MONTGOMERY COMO REFERÊNCIA BASE
\DIFadd{Uma das formas mais simples de aprendizado de máquina }\DIFaddend são \DIFdelbegin \DIFdel{métodos muito utilizados devido ao baixo custo computacional. Entretanto, têm a necessidade de estabelecer alguma }\DIFdelend \DIFaddbegin \DIFadd{modelos de regressão. Uma análise de regressão procura descrever as relações entre uma variável dependente (também chamada de variável de resposta) e variáveis independentes.
}

\DIFadd{Em particular, modelos de regressão linear são aqueles em que a }\DIFaddend relação \DIFdelbegin \DIFdel{(linearou quadrática) entre os termos estudados, utilizando termos fixos \mbox{%DIFAUXCMD
\cite{Bhadeshia1999}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{entre as variáveis dependente e independentes pode ser descrita por uma relação linear do tipo:
}

\begin{align}
  \DIFadd{y }&\DIFadd{= \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \dots + \beta_k x_k + \epsilon \nonumber }\\
    &\DIFadd{= \beta_0 + \sum_{i=1}^k \beta_i x_i + \epsilon
  \label{eq:regressao_linear}
}\end{align}
%DIF > 
\DIFadd{em que $y$ representa a variável dependente e $x_j$ e $\beta_j$ correspondem às variáveis dependentes e aos coeficientes de regressão.
}
...
\begin{equation}
  \DIFadd{y = \beta_0 + \sum_{i=1}^k \beta_i x_i + \sum_{i=1}^{k} \sum_{j=i}^k \beta_{ij} x_i x_j + \epsilon
  \label{eq:regressao_polinomial_2a_ordem}
}\end{equation}

\DIFadd{Note-se que o modelo descrito pela equação \ref{eq:regressao_polinomial_2a_ordem} também se trata de um modelo de regressão linear, uma vez que os termos de segunda ordem podem ser redefinidos como novas variáveis independentes de primeira ordem. De forma a ilustrar melhor esta situação, tome-se como exemplo o seguinte modelo de segunda ordem com duas variáveis $x_1$ e $x_2$:
}

\begin{equation}
  \DIFadd{y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_{11} }{\DIFadd{x_1}}\DIFadd{^2 + \beta_{22} }{\DIFadd{x_2}}\DIFadd{^2 + \beta_{12} x_1 x_2 + \epsilon
  \label{eq:regressao_polinomial_2a_ordem_2_var}
}\end{equation}

\DIFadd{Definindo $x_3 = {x_1}^2$, $x_4 = {x_2}^2$, $x_5 = x_1 x_2$, $\beta_3 = {\beta_1}^2$, $\beta_4 = {\beta_2}^2$ e $\beta_5 = \beta_1 \beta_2$, então a equação \ref{eq:regressao_polinomial_2a_ordem_2_var} se torna:
}

\begin{equation}
  \DIFadd{y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_3 + \beta_4 x_4 + \beta_5 x_5 + \epsilon 
}\end{equation}
%DIF > 
\DIFadd{que possui a mesma forma que a equação \ref{eq:regressao_linear} do modelo de regressão linear}\DIFaddend .

\DIFaddbegin \textit{\DIFadd{(Falar um pouco sobre método dos mínimos quadrados)}}

%DIF >  The method of least squares is typically used to estimate the regression coefficients in a mul-
%DIF >  tiple linear regression model. Suppose that n 
%DIF >   k observations on the response variable are
%DIF >  available, say y 1 , y 2 , . . . , y n . Along with each observed response y i , we will have an observa-
%DIF >  tion on each regressor variable and let x ij denote the ith observation or level of variable x j .
...
\DIFaddend A primeira etapa para elaboração de um algoritmo de \DIFdelbegin \DIFdel{machine learning }\DIFdelend \DIFaddbegin \DIFadd{aprendizado de máquina }\DIFaddend é a construção do banco de dados utilizado em seu treinamento. Para este trabalho, utilizou-se dados extraídos do software Thermo-Calc\textregistered{}\DIFaddbegin \DIFadd{, devido à sua acessibilidade}\DIFaddend .
...
Definidas as faixas de composição química e temperatura, foram definidos os níveis para cada elemento, ou seja, quantas variações (ou \DIFdelbegin \DIFdel{steps}\DIFdelend \DIFaddbegin \DIFadd{passos}\DIFaddend ) cada elemento tem. O valor do \DIFdelbegin \DIFdel{step }\DIFdelend \DIFaddbegin \DIFadd{passo }\DIFaddend é dado pela equação a seguir:

\begin{equation}
  \DIFdelbegin \DIFdel{step }\DIFdelend \DIFaddbegin \DIFadd{passo }\DIFaddend = \frac{\Delta c}{n - 1}
\end{equation}

Assim, os níveis e \DIFdelbegin \DIFdel{steps }\DIFdelend \DIFaddbegin \DIFadd{passos }\DIFaddend utilizados para cada elemento são dados na Tabela \DIFdelbegin \DIFdel{\ref{tab:niveis_e_steps}}\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:niveis_e_passos}}\DIFaddend .

\begin{table}
  \caption{Níveis e \DIFdelbeginFL \DIFdelFL{steps }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{passos }\DIFaddendFL para cada elemento de liga}

  \begin{tabular}{c c c}
  \hline
  \textbf{Elemento de liga} & \textbf{Níveis} & \textbf{Valor do \DIFdelbeginFL \DIFdelFL{step}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{passo}\DIFaddendFL } \\
  \hline
  Carbono & 11 & 0,15 \\
  Manganês & 5 & 0,75 \\
  Silício & 5 & 0,75 \\
  ...
  \DIFdelbeginFL %DIFDELCMD < \label{tab:niveis_e_steps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{tab:niveis_e_passos}
\DIFaddendFL \end{table}

Já para a temperatura, estabeleceu-se um \DIFdelbegin \DIFdel{step }\DIFdelend \DIFaddbegin \DIFadd{passo }\DIFaddend de 10K. A partir da combinação desses valores de composição, um script faz a chamada do Thermo-Calc\textregistered{}. Dessa forma, para dada composição química, são retornadas as porcentagens de cada fase (ferrita, austenita e cementita) para cada temperatura dentro da faixa estabelecida. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend O resultado da chamada do Thermo-Calc\textregistered{} é salvo em um arquivo de texto de extensão \DIFdelbegin \DIFdel{.DAT}\DIFdelend \DIFaddbegin \DIFadd{``.DAT''}\DIFaddend . No total, foram gerados \DIFdelbegin \DIFdel{6874 }\DIFdelend \DIFaddbegin \DIFadd{6875 }\DIFaddend arquivos.

\subsection{Extra\c{c}\~ao de temperaturas cr\'iticas}

Para cada arquivo gerado pela chamada do Thermo-Calc\textregistered{}, \DIFdelbegin \DIFdel{calculou-se }\DIFdelend as temperaturas críticas \DIFdelbegin \DIFdel{através }\DIFdelend \DIFaddbegin \DIFadd{foram calculadas por meio }\DIFaddend de outro script. Este faz a leitura do arquivo .DAT, que contém as porcentagens de cada fase para cada temperatura entre 673 e 1473K, variando em 10K.

Para determinar a A1, identifica-se a maior temperatura em que a porcentagem de austenita é zero, enquanto \DIFdelbegin \DIFdel{para o }\DIFdelend \DIFaddbegin \DIFadd{que para a temperatura }\DIFaddend A3, identifica-se a menor temperatura em que a porcentagem de austenita é \DIFdelbegin \DIFdel{um. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{100\%. }\DIFaddend Também identificou-se a temperatura crítica intermediária, A1', e \DIFdelbegin \DIFdel{consequentemente }\DIFdelend se o aço \DIFdelbegin \DIFdel{do respectivo arquivo }\DIFdelend \DIFaddbegin \DIFadd{da respectiva simulação }\DIFaddend é hipo ou hipereutetóide. Para isso, comparou-se a temperatura em que a porcentagem de ferrita é zero ($T_{ferr}$) com a que a porcentagem de cementita é zero ($T_{cem}$). Caso $T_{ferr}$ seja maior que $T_{cem}$, A1' é igual a $T_{cem}$ e o aço é hipoeutetóide; caso contrário, A1' é igual a $T_{ferr}$ e o aço é hipereutetóide. \DIFdelbegin \DIFdel{A terceira hipótese }\DIFdelend \DIFaddbegin \DIFadd{Uma terceira situação }\DIFaddend é \DIFaddbegin \DIFadd{a aquela em }\DIFaddend que não \DIFdelbegin \DIFdel{houvesse }\DIFdelend \DIFaddbegin \DIFadd{há }\DIFaddend cementita para a composição dada \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{e }\DIFaddend assim não \DIFdelbegin \DIFdel{haveria }\DIFdelend \DIFaddbegin \DIFadd{há }\DIFaddend campo trifásico e A1' seria igual a A1.
...
Em seguida, para cada arquivo, extraiu-se as temperaturas críticas A1, A1' e A3. \DIFdelbegin \DIFdel{As imagens a seguir ilustram }\DIFdelend \DIFaddbegin \DIFadd{A Figura \ref{fig:Tcrit_exemplos} ilustra }\DIFaddend a lógica dessa extração. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \begin{figure}
%DIFDELCMD <   \includegraphics[width=.9\textwidth]{img/714editado.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Extração de temperaturas críticas para aço hipoeutetóide}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:Tcrit_liga_hipo}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}
%DIFDELCMD <   \includegraphics[width=.9\textwidth]{img/4321editado.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Extração de temperaturas críticas para aço hipereutetóide}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:Tcrit_liga_hiper}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}
%DIFDELCMD <   \includegraphics[width=.9\textwidth]{img/418editado.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Extração de temperaturas críticas para aço hipoeutetóide sem cementita}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:Tcrit_liga_sem_cementita}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
\DIFaddbegin \begin{figure}
  ...
  \caption{\DIFaddFL{Exemplos de extração de temperaturas críticas para a) liga hipoeutetóide, b) liga hipereutetóide e c) liga hipoeutetóide sem cementita}}
  \label{fig:Tcrit_exemplos}
\end{figure}


\DIFaddend É importante destacar que nem sempre um aço terá as três temperaturas críticas. Elementos muito alfagênicos podem não ter A3, como no caso de um $\gamma$ loop, e gamagênicos podem não ter A1, por terem austenita estável à temperatura ambiente.
...
\DIFdelbegin \DIFdel{Também foi }\DIFdelend %DIF >  \subsection{Comparação com equações empíricas}
\DIFaddbegin 
...
Nota-se que, mesmo com a quantidade de dados reduzida, esses tiveram um bom \DIFdelbegin \textit{\DIFdel{fit}} %DIFAUXCMD
...
\subfloat[\label{fig:loss6}]{\includegraphics[width=.5\textwidth]{img/loss_6.png}}\DIFaddbeginFL \\
